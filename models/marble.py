...
class Marble(Sprite):
    ...
    def draw_selection_circle(self, win):
        ...
        pygame.draw.circle(win, color=RED, center=self.xy_center, radius=MARBLE_SIZE/2, width=2)

    def recalc_position(self):
        self.xy_pos = [val - MARBLE_SIZE / 2 for val in CoordinateHelper.from2DArraytoXY(position_2d=self.position_2d)]
        self.xy_center = [val for val in CoordinateHelper.from2DArraytoXY(position_2d=self.position_2d)]
        self.rect = pygame.Rect(self.xy_pos[0], self.xy_pos[1], MARBLE_SIZE, MARBLE_SIZE)
